...
export default function QuoteEditor({ initialQuoteData, clients = [], items = [], onSave, onCancel }) {
    ...
    const handleAddItemFromCatalog = (itemFromCatalog) => {
        const newItem = { id: Date.now(), description: itemFromCatalog.description, quantity: 1, price: itemFromCatalog.price };
    ...
    return (
        <div style={{fontFamily:'Inter, sans-serif', maxWidth:'900px', margin:'2rem auto', padding:'2rem', backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)'}}>
            ...
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid #e2e8f0', paddingBottom:'1rem', marginBottom:'1.5rem'}}>
                <div><h1 style={{margin:0}}>Quote Editor</h1><p style={{margin:0, color:'#718096'}}>Quote #{quote.quoteNumber}</p></div>
                <div>
            ...
            <div>
                ...
                {(quote.lineItems || []).map(item => (
                    <div key={item.id} style={{display:'grid', gridTemplateColumns:'4fr 1fr 1fr 0.5fr', gap:'1rem', alignItems:'center', marginBottom:'1rem'}}>
                        ...
                        <Input label="Qty" type="number" value={item.quantity} onChange={e => handleItemChange(item.id, 'quantity', e.target.value)} />
                        <Input label="Price" type="number" value={item.price} onChange={e => handleItemChange(item.id, 'price', e.target.value)} />
